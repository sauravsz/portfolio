# Saurav | Personal Portfolio & Systems Landing Page
...
## 🚀 Deployment (Netlify)
This repository is configured for zero-configuration static deployment on **Netlify**:
1. Connect this GitHub repository to Netlify.
2. Build settings are auto-configured via `netlify.toml`:
   - **Publish directory:** `.`
   - **Build command:** *(none required - pure high-performance static HTML/Tailwind)*

## 📐 Design & Stack
- Soft Structuralism & Ethereal OLED Material (`#07090E` / `#0C0F17`).
- Double-bezel hardware enclosures (Doppelrand nested architecture).
- Interactive real-time lead-time replenishment simulator.
- Typography: `Geist`, `Plus Jakarta Sans`, `Newsreader` (Italic Serif), `JetBrains Mono`.
- Lucide icon system.
